...
function toggleSpecificInputFields(elementID, displayStyle) {
  ...
  //event listeners for character and starhip dropdown menus
  ...
  document.getElementById("starshipRequestType").addEventListener("click", () => {
    ...
    if (starshipValue === "all") {
      ...
      fetchSpecificStarship(starshipName);
    } else if(starshipValue === "random") {
      fetchRandomStarship();
    ...
    fetch(`https://www.swapi.tech/api/people/?name=${characterName}`)
      ...
      .then((data) => {
        if (data.results.length > 0) {
          displayCharacter(data.results[0].properties);
        } else {
          throw new Error("Character not found");
        }
      })
...
function fetchStarship(starshipId) {
  ...
  function displayCharacter(character) {
    ...
    contentDiv.innerHTML = `
      ...
      <button onclick="fetchCharacterStarships('${character.uid}')">View Starships</button>`;
  }
